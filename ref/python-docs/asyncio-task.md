...
<div class="contents" depth="1" local="">
...
`Coroutines <coroutine>` declared with the async/await syntax is the preferred way of writing asyncio applications. For example, the following snippet of code prints "hello", waits 1 second, and then prints "world":
...
- The `asyncio.create_task` function to run coroutines concurrently as asyncio `Tasks <Task>`.
...
Wrap the *coro* `coroutine <coroutine>` into a `Task` and schedule its execution. Return the Task object.
...
3.11 Added the *context* parameter.

</div>

<div class="versionchanged">

3.14 Added the *eager_start* parameter by passing on all *kwargs*.
...
The asyncio components that enable structured concurrency, like `asyncio.TaskGroup` and `asyncio.timeout`, are implemented using cancellation internally and might misbehave if a coroutine swallows `asyncio.CancelledError`. Similarly, user code should not generally call `uncancel <asyncio.Task.uncancel>`. However, in cases when suppressing `asyncio.CancelledError` is truly desired, it is necessary to also call `uncancel()` to completely remove the cancellation state.
...
An `asynchronous context manager <async-context-managers>` holding a group of tasks. Tasks can be added to the group using `create_task`. All tasks are awaited when the context manager exits.
...
Passes on all *kwargs* to `loop.create_task`
...
`~asyncio.Task.cancel` will be called on any tasks in the group that aren't yet done, as well as the parent (body) of the group. The task group context manager will exit *without* `asyncio.CancelledError` being raised.
...
If the body of the `async with` statement exits with an exception (so `~object.__aexit__` is called with an exception set), this is treated the same as if one of the tasks failed: the remaining tasks are cancelled and then waited for, and non-cancellation exceptions are grouped into an exception group and raised. The exception passed into `~object.__aexit__`, unless it is `asyncio.CancelledError`, is also included in the exception group. The same special case is made for `KeyboardInterrupt` and `SystemExit` as in the previous paragraph. There is an additional special case made only for the body of the `async with`: if it raises `GeneratorExit` and none of the other tasks raise exceptions that would be reported, then the `GeneratorExit` is reraised.

Task groups are careful not to mix up the internal cancellation used to "wake up" their `~object.__aexit__` with cancellation requests for the task in which they are running made by other parties. In particular, when one task group is syntactically nested in another, and both experience an exception in one of their child tasks simultaneously, the inner task group will process its exceptions, and then the outer task group will receive another cancellation and process its own exceptions.

In the case where a task group is cancelled externally and also must raise an `ExceptionGroup`, it will call the parent task's `~asyncio.Task.cancel` method. This ensures that a `asyncio.CancelledError` will be raised at the next `await`, so the cancellation is not lost.
...
Addition of the special case for `GeneratorExit`.

</div>

## Sleeping

<div class="function" async="">
...
3.13 Raises `ValueError` if *delay* is `~math.nan`.
...
3.10 Deprecation warning is emitted if no positional arguments are provided or not all positional arguments are Future-like objects and there is no running event loop.
...
*custom_task_constructor* must be a *callable* with the signature matching the signature of `Task.__init__ <Task>`. The callable must return a `asyncio.Task`-compatible object.

This function returns a *callable* intended to be used as a task factory of an event loop via `loop.set_task_factory(factory) <loop.set_task_factory>`).
...
Protect an `awaitable object <asyncio-awaitables>` from being `cancelled <Task.cancel>`.
...
3.10 Deprecation warning is emitted if *aw* is not Future-like object and there is no running event loop.
...
Return an `asynchronous context manager <async-context-managers>` that can be used to limit the amount of time spent waiting on something.
...
<div class="Timeout(when)">

An `asynchronous context manager <async-context-managers>` for cancelling overdue coroutines.
...
Wait for the *aw* `awaitable <asyncio-awaitables>` to complete with a timeout.
...
<div id="asyncio_example_waitfor">
...
3.7 When *aw* is cancelled due to a timeout, `wait_for` waits for *aw* to be cancelled. Previously, it raised `TimeoutError` immediately.
...
3.11 Raises `TimeoutError` instead of `asyncio.TimeoutError`.

</div>

<div class="versionchanged">

3.12 Implemented using `asyncio.timeout`, a coroutine passed as *aw* is no longer wrapped in a `Task` when *timeout* is positive.
...
Run `~asyncio.Future` and `~asyncio.Task` instances in the *aws* iterable concurrently and block until the condition specified by *return_when*.
...
If `wait()` is cancelled, the futures in *aws* are not cancelled and continue to run.
...
3.11 Passing coroutine objects to `wait()` directly is forbidden.

</div>

<div class="versionchanged">

3.12 Added support for generators yielding tasks.
...
Run `awaitable objects <asyncio-awaitables>` in the *aws* iterable concurrently. The returned object can be iterated to obtain the results of the awaitables as they finish.
...
3.10 Deprecation warning is emitted if not all awaitable objects in the *aws* iterable are Future-like objects and there is no running event loop.
...
3.13 The result can now be used as either an `asynchronous iterator` or as a plain `iterator` (previously it was only a plain iterator).
...
See the `concurrency and multithreading <asyncio-multithreading>` section of the documentation.
...
<div class="Task(coro, *, loop=None, name=None, context=None, eager_start=False)">

A `Future-like <Future>` object that runs a Python `coroutine <coroutine>`. Not thread-safe.
...
Tasks are `generic <generics>` over the return type of their wrapped coroutines.

<div class="versionchanged">

3.7 Added support for the `contextvars` module.
...
3.12 Added the *eager_start* parameter.
...
> [!NOTE]
> This will return `None` for Tasks which have already completed eagerly. See the `Eager Task Factory <eager-task-factory>`.
...
3.11 The `msg` parameter is propagated from cancelled task to its awaiter.

</div>

<div id="asyncio_example_task_cancel">
...
This method is used by asyncio's internals and isn't expected to be used by end-user code. In particular, if a Task gets successfully uncancelled, this allows for elements of structured concurrency like `taskgroups` and `asyncio.timeout` to continue running, isolating cancellation to the respective structured block. For example:
...
3.13 Changed to rescind pending cancellation requests upon reaching zero.
