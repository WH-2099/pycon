...
PEP: 384 Title: Defining a Stable ABI Author: Martin von Löwis \<martin@v.loewis.de\> Status: Final Type: Standards Track Created: 17-May-2009 Python-Version: 3.2 Post-History:
...
- PyThreadState
- PyInterpreterState
- struct <span id="frame">frame</span>
- struct symtable
- struct <span id="node">node</span>
- PyWeakReference
- PyLongObject
- PyTypeObject
...
Function-like macros (in particular, field access macros) remain available to applications, but get replaced by function calls (unless their definition only refers to features of the ABI, such as the various <span id="check">Check</span> macros)
...
All functions starting with <span id="py">Py</span> are not available to applications. Also, all functions that expect parameter types that are unavailable to applications are excluded from the ABI, such as PyAST_FromNode (which expects a `node*`).
...
Module and type initializer and finalizer functions are not available (PyByteArray_Init, PyOS_FiniInterrupts and all functions ending in <span id="fini">Fini</span> or <span id="clearfreelist">ClearFreeList</span>).
...
A complete list of global variable definitions is given in the python3.def file[^2]; those declared DATA denote variables.
...
Also on Unix, the `3149` tag abi\<PYTHON_ABI_VERSION\> is accepted in file names of extension modules. No checking is performed that files named in this way are actually restricted to the limited API, and no support for building such files will be added to distutils due to the distutils code freeze.

# Implementation Strategy

This PEP will be implemented in a branch[^3], allowing users to check whether their modules conform to the ABI. To avoid users having to rewrite their type definitions, a script to convert C source code containing type definitions will be provided[^4].
...
[^1]: "python3 module definition file": <http://svn.python.org/projects/python/branches/pep-0384/PC/python3.def>

[^2]: "python3 module definition file": <http://svn.python.org/projects/python/branches/pep-0384/PC/python3.def>

[^3]: "PEP 384 branch": <http://svn.python.org/projects/python/branches/pep-0384/>

[^4]: "ABI type conversion script": <http://svn.python.org/projects/python/branches/pep-0384/Tools/scripts/abitype.py>
