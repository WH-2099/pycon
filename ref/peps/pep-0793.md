...
PEP: 793 Title: PyModExport: A new entry point for C extension modules Author: Petr Viktorin \<encukou@gmail.com\> Discussions-To: <https://discuss.python.org/t/93444> Status: Final Type: Standards Track Created: 23-May-2025 Python-Version: 3.15 Post-History: [14-Mar-2025](https://discuss.python.org/t/84498/), [27-May-2025](https://discuss.python.org/t/93444/), Resolution: [23-Oct-2025](https://discuss.python.org/t/93444/46)

<div class="canonical-doc">

`py3.15:extension-modules`
...
To make this viable, we also specify new module slot IDs to replace `PyModuleDef`'s fields, and to allow adding a *token* similar to the `Py_tp_token` used for type objects.
...
2.  A `PyModuleDef` object containing a description of how to create a module object. This option, *multi-phase initialization*, was introduced in `489`; see its motivation for why it exists.
...
- Adding a new export hook, which does not return a `PyObject*`.
  ...
  This was considered already for Python 3.5 in `489`, but rejected:
...
The `PyModuleDef_Slot` struct does have some downsides compared to fixed fields. We believe these are fixable, but leave that out of scope of this PEP. (Note: this was done in `820`, still in Python 3.15.)
...
> [!NOTE]
> `820` changed the return type to `PySlot *`.
...
> [!NOTE]
> `820` changed the first argument type to `PySlot *`.

The *slots* argument must point to an array of `PyModuleDef_Slot` structures, terminated by a slot with `slot=0` (typically written as `{0}` in C). The `Py_mod_abi` slot is required (see `803`); all other slots are optional. It follows that *slots* must not be `NULL`.
...
> [!NOTE]
> This is specialized functionality meant replace the `PyType_GetModuleByDef` function; users that don't need `PyType_GetModuleByDef` will most likely not need tokens either.
>
> This section contains the technical specification; for an example of intended usage, see `exampletype_repr` in the `Example section <pep793-example>`.
...
(Typically, it should be the slots array or `PyModuleDef` that a module is created from, or another static constant for dynamically created modules.)
...
A new `PyType_GetModuleByToken` function will be added, with a signature like the existing `PyType_GetModuleByDef` but a `const void *token` argument, and the same behaviour except matching tokens rather than only defs, and returning a strong reference.
...
For each field of the `PyModuleDef` struct, except ones from `PyModuleDef_HEAD_INIT`, a new slot ID will be provided: `Py_mod_name`, `Py_mod_doc`, `Py_mod_clear`, etc. Slots related to the module state rather than the module object will use a `Py_mod_state_` prefix. See `pep793-api-summary` for a full list.
...
A `PyMODEXPORT_FUNC` macro will be added, similar to the `PyMODINIT_FUNC` macro but with `PySlot *` as the return type.
...
The `PyInit_*` export hook will be `soft-deprecated <pep387-soft-deprecation>`.

## New API summary

> [!NOTE]
> This summary was updated with a change from `820`.
...
> [!NOTE]
> The guide is available at `py3.15:abi3t-howto-modexport`. (It is part of the `abi3t` migration HOWTO since switching to `PyModExport` doesn't bring benefits in 3.15 if you don't also adopt `abi3t`.)
>
> This section contains the original, outdated guide.

This guide is meant for hand-written modules. For code generators and language wrappers, the `pep793-shim` below may be more useful.
...
6.  Add slots for all members of the existing `PyModuleDef` structure. See `pep793-api-summary` for a list of the new slots. For example, to add a name and docstring:
...
> [!NOTE]
> This section was updated for `820`.

When compiled together with the `pep793-example` below on a non-free-threaded build of Python 3.15, the resulting extension is compatible with non-free-threading 3.11+ builds, in addition to a free-threading build of the reference implementation. (The module must be named without a version tag, e.g. `examplemodule.so`, and be placed on `sys.path`.)

Full support for creating such modules will require backports of some new API, and support in build/install tools. This is out of scope of this PEP. (In particular, the demo “cheats” by using a subset of Limited API 3.15 that *happens to work* on 3.11, and includes a few hacks. A proper implementation would use Limited API 3.11 with cleaner backport shims for new API like `Py_mod_name`.)
...
<div class="literalinclude" language="c">
...
In addition to regular reference docs, the `pep793-porting-notes` should be added as a new HOWTO.

# Example

> [!NOTE]
> The example was updated for `820`.
...
Implementation is tracked in [GitHub issue \#140550](https://github.com/python/cpython/issues/140550).

A draft implementation was available in a [GitHub branch](https://github.com/python/cpython/compare/main...encukou:cpython:modexport#files_bucket).
...
It is possible to change `PyModuleDef` to no longer include the `PyObject` header, and continue using the current `PyInit_*` hook. There are several issues with this approach:

- The import machinery would need to examine bit-patterns in the objects to distinguish between different memory layouts:

  - the “old” `PyObject`-based `PyModuleDef`, returned by current `abi3` extensions,
  - the new `PyModuleDef`,
  - `PyObject`-based module objects, for single-phase initialization.

  This is fragile, and places constraints on future changes to `PyObject`: the memory layouts need to stay *distinguishable* until both single-phase initialization and the current Stable ABI are no longer supported.

- `PyModuleDef_Init` is documented to “Ensure a module definition is a properly initialized Python object that correctly reports its type and a reference count.” This would need to change without warning, breaking any user code that treats `PyModuleDef`s as Python objects.
...
> [!NOTE]
> This idea was implemented in `820`.
...
[^1]: A [quick survey](##REF##quick survey) of multiple `Py_mod_exec` slots found zero uses in the top 15,000 PyPI projects, and three in the stardard library (including tests for the feature). The easy workaround is consolidating the `exec` functions; see `pep793-porting-notes` for details.
