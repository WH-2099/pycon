...
PEP: 525 Title: Asynchronous Generators Author: Yury Selivanov \<yury@edgedb.com\> Discussions-To: <python-dev@python.org> Status: Final Type: Standards Track Created: 28-Jul-2016 Python-Version: 3.6 Post-History: 02-Aug-2016, 23-Aug-2016, 01-Sep-2016, 06-Sep-2016

# Abstract

`492` introduced support for native coroutines and `async`/`await` syntax to Python 3.5. It is proposed here to extend Python's asynchronous capabilities by adding support for *asynchronous generators*.

# Rationale and Goals

Regular generators (introduced in `255`) enabled an elegant way of writing complex *data producers* and have them behave like an iterator.
...
Essentially, the goals and rationale for `255`, applied to the asynchronous execution case, hold true for this proposal as well.
...
This specification presumes knowledge of the implementation of generators and coroutines in Python (`342`, `380` and `492`).
...
The result of calling an *asynchronous generator function* is an *asynchronous generator object*, which implements the asynchronous iteration protocol defined in `492`.
...
`492` requires an event loop or a scheduler to run coroutines. Because asynchronous generators are meant to be used from coroutines, they also require an event loop to run and finalize them.
...
<img src="pep-0525-1.png" class="invert-in-dark-mode invert-in-dark-modealign-center" style="width:80.0%" alt="image" />
...
2.  When `PyAsyncGenASend.throw(*exc)` is called for the first time, `*exc` is thrown into the parent `agen` object.
...
2.  `sys.set_asyncgen_hooks()` and `sys.get_asyncgen_hooks()` methods to set up asynchronous generators finalizers and iteration interceptors in event loops.
...
Originally, `492` defined `__aiter__` as a method that should return an *awaitable* object, resulting in an asynchronous iterator.
...
`525` was accepted by Guido, September 6, 2016[^1].

# Implementation

The implementation is tracked in issue 28003[^2]. The reference implementation git repository is available at[^3].
...
[^1]: <https://mail.python.org/pipermail/python-dev/2016-September/146267.html>

[^2]: <http://bugs.python.org/issue28003>

[^3]: <https://github.com/1st1/cpython/tree/async_gen>
