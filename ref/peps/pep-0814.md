...
PEP: 814 Title: Add frozendict built-in type Author: Victor Stinner \<vstinner@python.org\>, Donghee Na \<donghee.na@python.org\> Discussions-To: <https://discuss.python.org/t/104854> Status: Final Type: Standards Track Created: 12-Nov-2025 Python-Version: 3.15 Post-History: [13-Nov-2025](https://discuss.python.org/t/104854) Resolution: [11-Feb-2026](https://discuss.python.org/t/pep-814-add-frozendict-built-in-type/104854/121)
...
See also `584` "Add Union Operators To dict".
...
> \>\>\> frozendict(x=1, y=2) frozendict({'x': 1, 'y': 2})
...
Since 2012 (`416`), the Python ecosystem has evolved:
...
`frozendict` now preserves the insertion order, whereas PEP 416 `frozendict` was unordered (as `603` `frozenmap`). `frozendict` relies on the `dict` implementation which preserves the insertion order since Python 3.6.
...
- Add `PyFrozenDictObject` structure which inherits from `PyDictObject` and has an additional `ma_hash` member.
...
We consider that such method can be added later if needed, but it doesn't have to be added right now. Moreover, if such method is added, it would be nice to add a similar method for `list`/`tuple` and `set`/`frozenset`. See also `351` (Freeze protocol).

## Type annotation

It [has been proposed](https://discuss.python.org/t/pep-814-add-frozendict-built-in-type/104854/37) to add `class TD(TypedDict, frozen=True)` or `Frozen[MyTypedDict]` to define a `frozendict` type.
...
- `416` (`frozendict`)
- `603` (`collections.frozenmap`)

# Acknowledgements

This PEP is based on prior work from Yury Selivanov (`603`).
