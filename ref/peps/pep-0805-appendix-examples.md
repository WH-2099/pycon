...
This example shows how an object can be made to appear as a synchronized object, usable across multiple ThreadGroups, by using the `protect` mechanism.
...
Protection does not guarantee thread safety, it merely enforces the locking discipline. While this makes it harder to accidentally make code that is thread unsafe, it doesn't make it impossible. In this example, the `increment` method is not thread safe as another thread might modify the value between the get and the set.
